...
async function fetchDashboardOverview(): Promise<DashboardOverviewResponse> {
  ...
  if (!res.ok) throw new Error(`Dashboard overview request failed: ${res.status}`)
  return res.json()
}
...
export function MatrixBudgetCockpit({
  ...
  const config = useMemo(
    () => resolveMatrixBudgetConfig(settings, import.meta.env as Record<string, unknown>),
    [settings],
  )
  ...
  const activeModel = contextQuery.data?.model?.trim() || workspaceModel || 'Unknown model'
  const analytics = overviewQuery.data?.analytics ?? null
  const spend = {
    estimatedCostUsd:
      typeof analytics?.estimatedCostUsd === 'number' ? analytics.estimatedCostUsd : null,
  ...
  const providerWarningText =
    summary.providerWarnings.length > 0
      ? `${summary.providerWarnings.length} provider${summary.providerWarnings.length === 1 ? '' : 's'} need attention`
      : providerStatuses.length > 0
        ? `${providerStatuses.length} provider checks healthy`
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-[var(--theme-border)] bg-[var(--theme-card)] px-2.5 py-1 text-[10px] uppercase tracking-[0.14em] text-[var(--theme-muted)]">
          ...
          <code className="text-[var(--theme-text)]">{config.nextEnforcementHook}</code>
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2 xl:grid-cols-3">
        <div className={cn('rounded-2xl border px-3 py-2.5', toneClasses(summary.contextState))}>
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em]">Context usage</div>
          <div className="mt-1 text-lg font-semibold text-[var(--theme-text)]">{Math.round(contextPercent)}%</div>
          <div className="mt-1 text-xs text-[var(--theme-text)]/80">
            {formatTokenCount(usedTokens)} / {formatTokenCount(maxTokens)} tokens
          </div>
          <div className="mt-1 text-[11px] text-[var(--theme-text)]/70">{activeModel}</div>
          <div className="mt-2 text-[10px] text-[var(--theme-text)]/70">
            Warns at {config.usageWarningPercent}% · advisory cap {config.advisoryContextLimitPercent}%
          </div>
        </div>

        <div className={cn('rounded-2xl border px-3 py-2.5', toneClasses(summary.spendState))}>
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em]">Spend / cost</div>
          <div className="mt-1 text-lg font-semibold text-[var(--theme-text)]">{formatUsd(summary.estimatedSpendUsd)}</div>
          <div className="mt-1 text-xs text-[var(--theme-text)]/80">
            30d dashboard estimate · {formatCostLabel(summary.costLabel)}
          </div>
          <div className="mt-2 text-[10px] text-[var(--theme-text)]/70">
            Warning {formatUsd(config.advisorySpendWarningUsd)} · limit {formatUsd(config.advisorySpendLimitUsd)}
          </div>
          <div className="mt-1 text-[10px] text-[var(--theme-text)]/70">
            Budget model {config.preferredBudgetModel || 'auto / provider default'}
          </div>
        </div>

        <div className={cn('rounded-2xl border px-3 py-2.5', toneClasses(summary.providerState === 'warning' ? 'warning' : 'ok'))}>
          <div className="text-[10px] font-semibold uppercase tracking-[0.14em]">Provider auth</div>
          <div className="mt-1 text-sm font-semibold text-[var(--theme-text)]">{providerWarningText}</div>
          <div className="mt-2 space-y-1 text-[11px] text-[var(--theme-text)]/75">
            {summary.providerWarnings.length > 0 ? (
              summary.providerWarnings.slice(0, 3).map((provider) => (
                <div key={provider.provider}>
                  <span className="font-medium text-[var(--theme-text)]">{provider.displayName}</span>
                  {provider.message ? ` — ${provider.message}` : ` — ${provider.status.replaceAll('_', ' ')}`}
                </div>
              ))
            ) : (
              <div>Credentials look healthy for the providers the workspace could probe.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] text-[var(--theme-muted)]">
        ...
        {(contextQuery.isError || providerQuery.isError || overviewQuery.isError) ? (
